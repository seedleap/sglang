...
class MinWMVideoArchConfig(DiTArchConfig):
    """Architecture config for the MinWM causal Wan2.1-1.3B world model.

    Checkpoint keys follow the original Wan repo naming (same family as the
    LingBot-World checkpoint), so the mapping mirrors LingBot's minus the
    Plücker-camera modules, plus the PRoPE output projection ``prope_o``.
    """
    ...
    # Causal streaming layout (minWM stage4 eval config):
    # 4-frame blocks, 16-frame local attention window with a 4-frame sink,
    # cache positions remapped block-relative after the window saturates.
    local_attn_size: int = 16
    sink_size: int = 4
    num_frames_per_block: int = 4
    sliding_window_num_frames: int = 16
