...
assert.match(
  app,
  /if \(!message\.type && message\.content_type\) \{\s*pendingHeader = message;/,
  "a structurally valid legacy frame header may arm the binary payload path",
);
assert.doesNotMatch(
  app,
...
assert.doesNotMatch(
  app,
  /if \(message\.type === "media_chunk_complete"\) \{\s*pendingHeader = message;/,
