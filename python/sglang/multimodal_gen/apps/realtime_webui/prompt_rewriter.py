...
class WorldRuleCompletionOutput(BaseModel):
    ...
    model_config = ConfigDict(
        extra="forbid",
        json_schema_extra={"propertyOrdering": ["name", "prompt", "change_type"]},
    )
...
def build_world_rule_message(previous_prompt: str, rule_input: str, kind: str) -> str:
...
def _validate_world_rule_output(payload: Any) -> WorldRuleCompletionOutput:
    ...
    if isinstance(payload, (bytes, bytearray)):
        return WorldRuleCompletionOutput.model_validate_json(payload.decode("utf-8"))
...
class PromptRewriter:
    ...
    async def complete_world_rule(
        ...
        client = await self._get_client()
        user_message = build_world_rule_message(previous_prompt, rule_input, kind)
        attempt_message = user_message
