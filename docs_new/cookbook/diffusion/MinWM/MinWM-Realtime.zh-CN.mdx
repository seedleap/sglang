...
### 720p 三路 whole-DiT compile 复测

2026-07-23 的 H200 attempt 20 固定同一 0721 checkpoint、同一
`00_forward_080_pottery_720p` prompt/seed/首帧/连续 `w=0.8` action，按顺序生成：

1. minWM `main` / V3 baseline；
2. SGLang 已验收 bitwise lane；
3. SGLang 性能 lane：dense attention、SGLang VAE/T5、非确定性执行、关闭内层
   segment compile、打开 whole-DiT `module.compile`。

三支 MP4 均经 `ffprobe` 验证为 `1248x704`、24 FPS、129 帧、5.375 秒。Bitwise lane
与 baseline 的 lossless generated frames 仍逐字节相同：
`max_abs=0、RMSE=0、SSIM=1`。性能 lane 有意不要求 parity，其 generated-frame
指标为 `RMSE=29.4657、SSIM=0.838568、PSNR=18.7445 dB`。

| Lane | Steady client FPS | Scheduler FPS | Warm TTFF | Peak GPU |
| --- | ---: | ---: | ---: | ---: |
| SGLang bitwise | 10.822 | 10.799 | 1.976 s | 53,159 MiB |
| optimized eager control | 10.853 | 10.817 | 1.900 s | 未单独采样 |
| optimized whole-DiT compiled | 12.711 | 12.643 | 1.701 s | 51,269 MiB |

保持 optimized profile 其他设置不变时，whole-DiT compile 单独带来 `17.12%` 的
steady client FPS 增量；完整 compiled speed lane 相对 bitwise lane 提升 `17.46%`。
首次 compiled chunk 的 scheduler 时间为 `235.6 s`，完整 warmup 请求约四分钟，
所以这个配置适合常驻服务，不适合频繁冷启动。

这里的 whole-DiT 是对 transformer module 调用 `module.compile`；SGLang 当前参数是
`fullgraph=False`，允许内部 graph break。它不等于单一无断点巨图。性能 lane 关闭
MinWM 的小粒度动态 segment compile，是为了避免外层和内层嵌套编译边界。这个组合
不再复现早期 `FakeTensor * Node`，但由于同时改变了多个配置，只能说明该失败可规避，
不能从这一次实验断言 segment compile 是唯一根因。

本地三路同步播放器和报告位于
`benchmark/minwm_realtime_parity/results/latest-checkpoint-720p-triptych-h200/latest/`。
`player/index.html` 同屏展示三路，并提供一键播放/暂停、同步 seek、倍速和逐帧控制。
...
纯 T2V 不上传首帧，使用最终 pixel-frame 数固定完整 horizon。例如 24 FPS、约 5 秒
使用 121 帧：
...
MinWM T2V 要求 `num_frames = 1 + N * 4`；adapter 会据此推导第一块、常规块、
尾块和 `max_chunks`。新客户端应显式传 `generation_mode`；旧客户端不传时仍按
`first_frame` 是否存在推断，因此原有 I2V/parity 脚本不需要同步升级。Realtime
Studio 仅在部署配置的 `generationModes` 包含 `t2v` 时显示模式切换；T2V 下会隐藏
Reference，并确保 init 包不含 `first_frame`。
...
默认 `--realtime-vae-backend local` 保持原生 causal VAE 在 Denoiser 进程内，不会自动
切换画质。需要第二张 GPU overlap 时，显式选择 `exact_remote` 或
`taehv_remote`，并让统一 worker 选择匹配的 `exact` 或 `taehv` decoder：

```bash
# GPU 1：原生 exact causal VAE；模型级 cache 只允许一个活动 Session。
...
# GPU 0：把 latent 交给上面的 exact worker。
...
同机 loopback 的 `auto` 优先使用 `/dev/shm/sglang-realtime-vae`；容器部署必须让两个
进程看到同一路径，或在两侧同时显式设置 `--shared-memory-dir` 与
`--realtime-vae-shared-memory-dir`。跨主机使用 `--realtime-vae-transport websocket`。

TAEHV worker 把 `--decoder-backend exact --vae-path ...` 改为
`--decoder-backend taehv --checkpoint-path /models/taehv/taew2_2.pth`，Denoiser 改为
`--realtime-vae-backend taehv_remote`。TAEHV 是近似 decoder，不提供 bitwise 或
numerical parity 承诺。完整的历史关系、生命周期、T2V 裁剪、部署命令和 GPU 测量
gate 见 [Realtime VAE overlap 后端统一决策](/docs/sglang-diffusion/minwm/specs/2026-08-09-realtime-vae-backend-unification)。
...
| D34 | Whole-DiT compile 只在独立的非 parity 性能 profile 中开启，bitwise 默认继续关闭。 | 1248x704 性能 lane 现在能够完成模块级 compile，但 SGLang 使用 `fullgraph=False`，允许内部 graph break；该 lane 的输出也不再逐字节相同。产品默认仍只保留与 minWM `main` 相同的局部 fused-segment compile，不能用 eager exact 结论替 compiled lane 背书。 |
...
| D40 | Whole-DiT 性能 lane 关闭 MinWM 的小粒度动态 segment compile。 | 对整个 transformer 调用 `module.compile` 时，再在 AdaLN、RMSNorm 和 QK+RoPE 内嵌套 `torch.compile(dynamic=True)` 会形成重复 graph boundary 和 recompile。性能 lane 让 Inductor 管理外层模块，exact lane 则保留 source-shaped segment boundary；两种 scope 不混用。 |
| D41 | 三路页面固定同一 case，并把首次完整请求作为 warmup 后再计量。 | 首次 whole-DiT compile/autotune 是分钟级冷启动，若把它混入 5.375 秒 clip FPS，会掩盖服务稳态能力。页面同时保存 baseline、bitwise、optimized 的 lossless arrays、MP4、TTFF、steady FPS 和误差，而 optimized 的组合收益不会被误写成纯 compile 收益。 |
| D42 | artifact-ready 标记之后仍允许显式保留容器。 | Kubernetes `emptyDir` 会随 Pod 退出立即消失；第一次三路成功运行虽有完整日志和摘要，却无法事后恢复视频。`MINWM_ARTIFACT_HOLD_SECONDS` 将计算完成与人工/自动拷贝窗口分离，不改变模型运行。 |
| D43 | 第一阶段 causal Ulysses 只支持 `sp_degree == ulysses_degree > 1` 且 `ring_degree == 1`。 | MinWM 现在会对 patch/action token 做 contiguous shard，把完成 RoPE 的 Q/K/V 合并成一次 input all-to-all，每个 rank 只保存 full-history causal KV 的 local head，attention 后执行 reverse collective，并在 scheduler/VAE 前 gather output projection token。Head 必须可整除；Ring、TP+SP、FSDP+SP、whole-DiT compile+SP 继续拒绝。 |
| D44 | Realtime init 增加显式 `generation_mode=i2v|t2v`，但保留按首帧存在性推断的兼容路径。 | 旧 API 已能通过“缺少首帧”进入 T2V，却无法在前端表达，也无法尽早发现模式/输入矛盾。显式字段让 UI、API 和日志使用同一合同；兼容推断避免破坏已经用于 parity 的 MsgPack 重放脚本。 |

<Warning>
  Causal Ulysses 已有本地单测与隔离运行覆盖，但本文尚未记录 SP2/SP4/SP8 的 GPU parity
  和性能矩阵。不能用历史 SP1 结果替代多卡 bitwise 或加速结论。
</Warning>
...
32. 1248x704 的第一次 whole-DiT `torch.compile` 尝试在 Torch 2.11 Inductor
    post-grad 阶段因 `FakeTensor * Node` 报错。随后把外层模块 compile 与 MinWM
    内层动态 segment compile 分开，非 parity 性能 lane 能够完成编译和 129 帧推理。
    这说明早期失败不是“720p 永远不可编译”，但不能仅凭组合配置变化断言唯一根因。
    Compiled output 不再 bitwise，因此 exact 默认仍关闭 whole-DiT compile。
...
34. 第一次成功的三路编译运行同样暴露了 `emptyDir` 生命周期：计算和报告全部成功，
    但容器退出后视频不可恢复。重跑在同一 H200、同一 checkpoint 和同一 case 上验证
    哈希与吞吐，同时增加一小时只读抓取窗口；实现也将该窗口做成可选环境变量。
...
- 1248x704、原生全历史 KV 的同 case control 中，optimized eager 为 `10.853 FPS`，
  otherwise-identical whole-DiT compiled 为 `12.711 FPS`，所以 H200 720p 下 compile
  单项增量为 `17.12%`。Compiled 相对 bitwise 的 `17.46%` 是组合收益，不能反过来
  当作 compile 单项。
...
19. 三路页面的 optimized lane 同时改变了哪些执行边界？为什么它相对 bitwise 的
    `17.46%` 不能直接归因于 `torch.compile`？
20. 解释 `module.compile(fullgraph=False)` 与“单一无 graph break 的完整大图”之间的
    差别。为什么 performance lane 要关闭内层 segment compile，而 exact lane反而保留它？
...
第 16–20 题为每题四分的专题加分题，不改变原 100 分总分。
...
- 2026-07-23 — 将 whole-DiT compile 与 MinWM 内层动态 segment compile 分离：
  exact lane 保留 source-shaped segment compile 并关闭 whole-DiT；performance lane
  关闭内层 segment compile，对 transformer module 使用
  `max-autotune-no-cudagraphs`、`fullgraph=False`。1248x704 首图编译不再触发早期
  `FakeTensor * Node`，但输出不再 bitwise，因此没有修改产品默认值。
- 2026-07-23 — H200 attempt 19 首次完成 baseline/bitwise/compiled 三路计算，但
  Pod 退出后 `emptyDir` 立即回收，视频无法恢复。Attempt 20 在同一节点重跑并加入
  `MINWM_ARTIFACT_HOLD_SECONDS`；三支 MP4、lossless 指标、服务日志和显存采样均成功
  抓取。随后补跑 otherwise-identical optimized eager control，把 720p compile 单项
  增量定为 `17.12%`，而不是把组合 profile 的 `17.46%` 全部归因给 compile。
