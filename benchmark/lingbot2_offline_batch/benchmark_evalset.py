...
async def warmup_server(
    ...
    """Retry transient realtime-session warmup failures in place.

    LingBot can rarely lose the just-created realtime session before block 1.
    Restarting the whole pod for that transient discards an otherwise healthy,
    fully loaded server, so retry the warmup with a fresh websocket/session.
    """
